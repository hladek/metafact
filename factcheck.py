...
Respond ONLY with a JSON object (no markdown fences):
{
  ...
  "confidence": <0.0–1.0>,
  "summary": "<2–3 sentence summary of findings>",
  "justification": "<detailed explanation based on your knowledge>",
  "sources": []
}
...
Your task:
1. Search the web up to 10 times to gather evidence.
...
def verify_claim_search(claim: str, model: str = DEFAULT_MODEL) -> dict[str, Any]:
    ...
    Skips the reputable-source selection step; the LLM searches the open web
    directly via `search_web`. Faster than verify_claim but without curated
    source filtering.
